...
from .source.interfaces import (
    HashInterface,
    ConversorInterface,
    ExtractLogInfosInterface,
)

from .source.api_routes_class.interfaces import (
    DeleteFilesRouteInterface,
    UploadAudioRouteInterface,
    GetConvertedAudioRouteInterface,
    GetStatusFileRouteInterface,
)
...
def upload_audio():
    if request.method == "POST":
        upload_audio_route: UploadAudioRouteInterface = Fac.get_representative(
            UploadAudioRouteInterface
        )()
...
def get_converted_audio(filename: str):
    get_converted_audio_route: GetConvertedAudioRouteInterface = Fac.get_representative(
        GetConvertedAudioRouteInterface
    )()
    get_converted_audio_route.set_atributes(
        filename=filename, url_base=request.base_url
    )
...
def get_status_file(hash: str):
    ...
    get_status_file_route: GetStatusFileRouteInterface = Fac.get_representative(
        GetStatusFileRouteInterface
    )()
    get_status_file_route.set_atributes(
        path=f"{Path().absolute()}/status/", hash=hash, extract_log=extract_log
    )
